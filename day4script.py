        ...
        for product in state.order_depths.keys():
            ...
            if product == 'COCONUTS':
                ...
                if (len(order_depth_coconuts.sell_orders) > 0) and (len(order_depth_pina.buy_orders) > 0):
                    ...
                    # Check if the lowest ask (sell order) is lower than the above defined fair value
                    if comparable_bid_pinas > comparable_ask_coconuts + 0.2 and position < 200:
                ...
                if (len(order_depth_coconuts.buy_orders) != 0) and (len(order_depth_pina.sell_orders) > 0):
                    ...
                    if comparable_ask_pinas + 0.2 < comparable_bid_coconuts and position > -200:
            ...
            if product == 'BAGUETTE':
                ...
                # Retrieve the Order Depth containing all the market BUY and SELL orders for COCONUTS
                order_depth_BAGUETTE: OrderDepth = state.order_depths[product]
                order_depth_PICNIC_BASKET: OrderDepth = state.order_depths['PICNIC_BASKET']
                order_depth_UKULELE: OrderDepth = state.order_depths['UKULELE']
                order_depth_DIP: OrderDepth = state.order_depths['DIP']
                outbound_orders_BAGUETTE: list[Order] = []
                outbound_orders_PICNIC_BASKET: list[Order] = []
                outbound_orders_UKULELE: list[Order] = []
                outbound_orders_DIP: list[Order] = []

                try:
                    position_DIP = state.position['DIP']
                except:
                    position_DIP = 0

                try:
                    position_BAGUETTE = state.position['BAGUETTE']
                except:
                    position_BAGUETTE = 0

                try:
                    position_UKULELE = state.position['UKULELE']
                except:
                    position_UKULELE = 0
                
                try:
                    position_PICNIC_BASKET = state.position['PICNIC_BASKET']
                except:
                    position_PICNIC_BASKET = 0

                #print("coconuts position: ",position)

                print("Checking")
                print((len(order_depth_PICNIC_BASKET.sell_orders) > 0) and (len(order_depth_BAGUETTE.buy_orders) > 0) and (len(order_depth_DIP.buy_orders) > 0) and (len(order_depth_UKULELE.buy_orders) > 0))
                if (len(order_depth_PICNIC_BASKET.sell_orders) > 0) and (len(order_depth_BAGUETTE.buy_orders) > 0) and (len(order_depth_DIP.buy_orders) > 0) and (len(order_depth_UKULELE.buy_orders) > 0):
                    print("We're through")
                    # Sort all the available sell orders by their price,
                    # and select only the sell order with the lowest price
                    best_bid_PICNIC_BASKET = max(order_depth_PICNIC_BASKET.buy_orders.keys())
                    best_bid_volume_PICNIC_BASKET = abs(order_depth_PICNIC_BASKET.buy_orders[best_bid_PICNIC_BASKET])
                    best_ask_BAGUETTE = abs(min(order_depth_BAGUETTE.sell_orders.keys()))
                    best_ask_volume_BAGUETTE = abs(order_depth_BAGUETTE.sell_orders[best_ask_BAGUETTE])
                    best_ask_DIP = min(order_depth_DIP.sell_orders.keys())
                    best_ask_volume_DIP = abs(order_depth_DIP.sell_orders[best_ask_DIP])
                    best_ask_UKULELE = min(order_depth_UKULELE.sell_orders.keys())
                    best_ask_volume_UKULELE = abs(order_depth_UKULELE.sell_orders[best_ask_UKULELE])  # check correct logic


                    price_sell_PICNIC_BASKET = best_bid_PICNIC_BASKET

                    price_buy_components = 2*best_ask_BAGUETTE + 4*best_ask_DIP + 1*best_ask_UKULELE

                    print("Price of components: ",price_buy_components," price basket: ",price_sell_PICNIC_BASKET)

                    if price_buy_components < price_sell_PICNIC_BASKET:
                        print("should buy sell here")

                        # check lowest volume among components

                        lowest_volume = 0

                        if best_ask_volume_BAGUETTE/2 >= best_ask_volume_UKULELE:
                            lowest_volume = best_ask_volume_UKULELE
                            product = 'UKULELE'
                            if best_ask_volume_DIP/4 >= lowest_volume :
                                pass
                            else:
                                lowest_volume = best_ask_volume_DIP / 4
                        else:
                            lowest_volume = best_ask_volume_BAGUETTE
                            product = 'BAGUETTE'
                            if best_ask_volume_DIP/4 >= lowest_volume/2 :
                                lowest_volume = lowest_volume / 2
                            else:
                                lowest_volume = best_ask_volume_DIP / 4
                        
                        base_volume = lowest_volume

                        # check lowest among baskets

                        if base_volume > best_bid_volume_PICNIC_BASKET:
                            base_volume = best_bid_volume_PICNIC_BASKET
                        

                        if base_volume > 70:
                            base_volume = 70

                        # buy components, sell picnic basket

                        #print("SELL",'COCONUTS', str(-volume_coconuts_bid) + "x", best_bid_coconuts)
                        outbound_orders_PICNIC_BASKET.append(Order('PICNIC_BASKET', best_bid_PICNIC_BASKET, -base_volume)) # could bug here

                        # buying the pinas
                        #print("BUY", 'PINAS', str(-volume_pinas_ask) + "x", best_ask_pinas)
                        outbound_orders_BAGUETTE.append(Order('BAGUETTE', best_ask_BAGUETTE, 2*base_volume)) # removed negative here
                        outbound_orders_DIP.append(Order('DIP', best_ask_DIP, 4*base_volume)) # removed negative here
                        outbound_orders_UKULELE.append(Order('UKULELE', best_ask_UKULELE, base_volume)) # removed negative here
                
                    #########
                    best_ask_PICNIC_BASKET = min(order_depth_PICNIC_BASKET.sell_orders.keys())
                    best_ask_volume_PICNIC_BASKET = abs(order_depth_PICNIC_BASKET.sell_orders[best_ask_PICNIC_BASKET])
                    best_bid_BAGUETTE = max(order_depth_BAGUETTE.buy_orders.keys())
                    best_bid_volume_BAGUETTE = abs(order_depth_BAGUETTE.buy_orders[best_bid_BAGUETTE])
                    best_bid_DIP = max(order_depth_DIP.buy_orders.keys())
                    best_bid_volume_DIP = abs(order_depth_DIP.buy_orders[best_bid_DIP])
                    best_bid_UKULELE = max(order_depth_UKULELE.buy_orders.keys())
                    best_bid_volume_UKULELE = abs(order_depth_UKULELE.buy_orders[best_bid_UKULELE])  # check correct logic


                    price_buy_PICNIC_BASKET = best_ask_PICNIC_BASKET

                    price_sell_components = 2*best_bid_BAGUETTE + 4*best_bid_DIP + 1*best_bid_UKULELE

                    if price_sell_components > price_buy_PICNIC_BASKET:

                        # check lowest volume among components

                        lowest_volume = 0

                        if best_bid_volume_BAGUETTE/2 >= best_bid_volume_UKULELE:
                            lowest_volume = best_bid_volume_UKULELE
                            product = 'UKULELE'
                            if best_bid_volume_DIP/4 >= lowest_volume :
                                pass
                            else:
                                lowest_volume = best_bid_volume_DIP / 4
                        else:
                            lowest_volume = best_bid_volume_BAGUETTE
                            product = 'BAGUETTE'
                            if best_bid_volume_DIP/4 >= lowest_volume/2 :
                                lowest_volume = lowest_volume / 2
                            else:
                                lowest_volume = best_bid_volume_DIP / 4
                        ...
                        if base_volume > best_ask_volume_PICNIC_BASKET:
                            base_volume = best_ask_volume_PICNIC_BASKET
                        
                        if base_volume > 70:
                            base_volume = 70

                        # sell components, buy picnic basket

                        #print("BUY",'COCONUTS', str(-volume_coconuts_bid) + "x", best_bid_coconuts)
                        outbound_orders_PICNIC_BASKET.append(Order('PICNIC_BASKET', best_ask_PICNIC_BASKET, base_volume)) # could bug here

                        # selling the pinas
                        #print("sell", 'PINAS', str(-volume_pinas_ask) + "x", best_ask_pinas)
                        outbound_orders_BAGUETTE.append(Order('BAGUETTE', best_bid_BAGUETTE, -2*base_volume)) # removed negative here
                        outbound_orders_DIP.append(Order('DIP', best_bid_DIP, -4*base_volume)) # removed negative here
                        outbound_orders_UKULELE.append(Order('UKULELE', best_bid_UKULELE, -base_volume)) # removed negative here
                    #########
                    # Add all the above the orders to the result dict
                    #result['UKULELE'] = outbound_orders_UKULELE
                    #result['BAGUETTE'] = outbound_orders_BAGUETTE
                    #result['DIP'] = outbound_orders_DIP
                    #result['PICNIC_BASKET'] = outbound_orders_PICNIC_BASKET


                    # otherwise market make anyway
